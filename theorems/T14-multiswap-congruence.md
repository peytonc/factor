...
variables: [P_key, P_shadow, M_m, a_parent, b_parent]
status: proven
...
- Let `M_i` be the modular consequence of the i-th swap: if the swap is B vs C, `M_i = 2*a_parent_i`; if the swap is A vs B, `M_i = 2*b_parent_i`.
- Let `v_key` and `v_shadow` be the resulting leaf state vectors.
- `v_shadow ≡ v_key (mod GCD(M_1, M_2, ..., M_k))`.
